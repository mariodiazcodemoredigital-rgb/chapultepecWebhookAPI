...
// 1. CONFIGURACIÓN INICIAL DE SERILOG (Antes del builder)
Serilog.Debugging.SelfLog.Enable(msg => Console.WriteLine($"[SERILOG ERROR] {msg}"));

Log.Logger = new LoggerConfiguration()
    .MinimumLevel.Information()
    .MinimumLevel.Override("Microsoft", LogEventLevel.Warning)
    .Enrich.FromLogContext() // ESENCIAL para logs en controladores
    .WriteTo.Console()
    .WriteTo.File("Logs/webhook-audit-.txt",
        rollingInterval: RollingInterval.Day,
        buffered: false,
        shared: true, // Permite que varios hilos escriban al mismo tiempo
